...
```
用户提问 → 问题分类器（系统文档/运维知识/故障案例/图表文档/数据查询）
  → 数据查询类 → 走现有 Gateway Tools（不变）
  → 文档知识类（含图表） → BGE嵌入 → Chroma向量检索 → Top-K片段注入系统提示词 → LLM基于文档回答
```
...
| **chart_docs（图表文档）** | 完整参考手册（按▌分隔） | **64** | 所有图表/卡片的类型、数据源、解读指引 |
...
- **球球助手 RAG 增强**：`/api/assistant/explain` 调用时先检索 chart_docs，将图表文档注入 AI 上下文，支持粘贴任意图表标题即解释
