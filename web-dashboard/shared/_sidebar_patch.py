"""Convert navbar to left sidebar CSS"""
with open('navbar.js', 'r', encoding='utf-8') as f:
    content = f.read()

css_start = content.find('var css = [')
css_end = content.find('  ].join("\\n");', css_start) + len('  ].join("\\n");')

sidebar_css = r"""var css = [
    ':root {',
    ...
    '}',
    '#global-nav .gn-inner {',
    '  display: flex; flex-direction: column; height: 100%; padding: 16px 0; gap: 0;',
    '}',
    '#global-nav .gn-logo {',
    ...
    '  background: var(--gn-popup-bg);',
    '  border: 1px solid var(--gn-popup-border); border-radius: 12px;',
    '  box-shadow: 0 24px 64px rgba(0,0,0,0.15); z-index: 9999;',
    '  padding: 28px 32px 24px; font-size: 13px; color: var(--gn-text-secondary);',
...
content = content[:css_start] + sidebar_css + content[css_end:]
